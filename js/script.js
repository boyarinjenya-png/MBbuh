  ...
  if (serviceModal) {
    ...
    const catalog = {
      ...
      "liquid-too": {
        ...
        text: "Сопровождаем ликвидацию товарищества, финальную отчётность и снятие с учёта",
        items: ["подготовка документов на ликвидацию", "финальная отчётность", "закрытие обязательств", "снятие с учёта", "консультации на каждом этапе"],
      },
      charter: {
        title: "Изменения в учредительных документах",
        text: "Сопровождаем изменения в уставе и регистрационных данных компании",
        items: ["подготовка пакета изменений", "правки устава", "смена адреса или руководителя", "подача документов", "консультации по регистрации"],
      },
      hr: {
        title: "Кадровый учёт",
        text: "Ведем кадровые документы и сопровождаем трудовые отношения в компании",
        items: ["трудовые договоры", "приказы и кадровые формы", "учёт рабочего времени", "отпуска и увольнения", "консультации по кадрам"],
      },
      payroll: {
        title: "Зарплата сотрудников",
        text: "Рассчитываем заработную плату, налоги и обязательные отчисления",
        items: ["расчёт заработной платы", "ОПВ, СО, ОСМС", "налоги с фонда оплаты труда", "платёжные ведомости", "отчётность по сотрудникам"],
      },
    };
    ...
    const fill = (key, origin) => {
      ...
      const label = origin && origin.querySelector(".svc-card__num");
      const labelText = label ? label.textContent.trim() : "";
      iconEl.innerHTML = "";
      iconEl.textContent = labelText;
      markEl.innerHTML = "";
      markEl.textContent = labelText;
